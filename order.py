...
class Notepad: 
        ...
        def __init__(self,**kwargs):
                ...
                self.thisFontMenu.add_command(label="Modern", command=self.changeFont1)

                #self.thisMenuBar.add_cascade(label="Font",menu=self.thisFontMenu)
                self.thisFontMenu.add_command(label="Arial", command=self.changeFont2)
                self.thisMenuBar.add_cascade(label="Font",menu=self.thisFontMenu)
                self.thisMenuBar.add_command(label="Font1",command=self.wid)
                
                #self.thisFormatMenu.add_cascade(label="Style",menu=self.thisMenuBar)
                #self.thisFormatMenu.add_cascade(label="Style",menu=self.thisMenuBar)
        ...
        def changeFont1(self):
                self.bold_font = Font(family="Modern", size=14, weight="bold")#Applied bold just to see the difference.Otherwise use weight="normal"
                try:
                        print ("Inside try")
                        self.thisTextArea.tag_add("Modern", "sel.first", "sel.last")
                        print(self.thisTextArea.selection_get())
                        self.thisTextArea.tag_configure("Modern",font=self.bold_font)
                        print ("Font chagned")
                except tkinter.TclError:
                        print ("Exception")
...
notepad = Notepad(width=200,height=300) 
notepad.run1()
